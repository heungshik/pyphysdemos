...
#    advancer = EMDynamics.WeaveCDynamics(fields)
    ...
    #Set up a wire with current.
    fields.current[:,20,20,0] = 0.5
    ...
    for i in range(100):
        ...
        sizeo = 10.0 * np.where(viz.BZ>0, viz.BZ, 0.0).flatten()
        sizex = 10.0 * np.where(viz.BZ<0, -viz.BZ, 0.0).flatten()
