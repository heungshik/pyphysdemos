...
#    advancer = EMDynamics.PurePythonDynamics(fields)
    ...
    for i in range(100):
        ...
        plt.quiver(viz.EXX*1e2, viz.EXY*1e2, viz.EX, viz.zeros, width=0.002, 
                   scale=scale, color="blue", pivot="middle")
        plt.quiver(viz.EYX*1e2, viz.EYY*1e2, viz.zeros, viz.EY, width=0.002, 
                   scale=scale, color="blue", pivot="middle")
        
        sizeo = 20.0 / emax * np.where(viz.BZ>0, viz.BZ, 0.0).flatten()
        sizex = 20.0 / emax * np.where(viz.BZ<0, -viz.BZ, 0.0).flatten()
